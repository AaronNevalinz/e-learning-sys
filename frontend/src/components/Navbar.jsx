...
export default function Navbar() {
  ...
  return (
    <div className="shadow-xl h-16 fixed z-50 w-full bg-white">
      <nav className="flex justify-between items-center py-4 max-w-8xl px-10 mx-auto">
        <div className="flex gap-x-5">
          <Link
            className={
              "bg-gradient-to-r font-bold font-montserrat from-purple-500 via-red-500 to-blue-700 inline-block text-transparent bg-clip-text "
            }
            to="/"
          >
          ...
          {token && (
            <form action="" onSubmit={handleQuerySubmit}>
              <Input
                ...
                placeholder="Search..."
                className={"py-1 placeholder:text-sm"}
              />
            </form>
          )}
        </div>
        <div className="flex gap-x-10">
          <ul className="flex gap-x-10 items-center text-sm uppercase text-slate-800 hover:text-slate-700 transition-all font-semibold">
            <li>
              <Link to={"/"}>Home</Link>
            </li>
            <li>
